...
export default function SettingsPage() {
  ...
  const RESTAURANT_ID = "eaaefe2f-bd7d-4a4b-a40d-ee775ec44130"; // 🔴 PASTE UUID
  ...
  const handleAddStaff = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewStaff({ full_name: "", role: "cashier", pin_code: "" });
      fetchData();
    } catch (err: any) {
      alert("Gagal: " + err.message);
    }
          ...
          <div className="bg-white p-6 rounded-lg shadow border border-gray-200">
            ...
            <form onSubmit={handleSaveSettings} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="col-span-2 border-t pt-4 flex justify-end">
                <button type="submit" disabled={isSaving} className="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 font-bold">
            ...
            <div className="bg-white p-6 rounded-lg shadow border border-gray-200 h-fit">
              ...
              <form onSubmit={handleAddStaff} className="space-y-4">
                ...
                <input
                  ...
                  className="w-full border border-gray-300 rounded px-3 py-2 text-gray-900 font-mono"
                />
                ...
                <tbody>
                  {staffList.map((staff) => (
                    <tr key={staff.id} className="hover:bg-gray-50 border-b">
                      <td className="p-3 font-medium text-gray-900">{staff.full_name}</td>
                      <td className="p-3 uppercase text-xs">{staff.role}</td>
                      <td className="p-3 font-mono">****{staff.pin_code.slice(-2)}</td>
                      <td className="p-3">
                        <button onClick={() => handleDeleteStaff(staff.id)} className="text-red-600 hover:text-red-800 text-xs font-bold">
                          Hapus
                        </button>
                      </td>
                    </tr>
              ...
              <ul className="space-y-2">
                {paymentMethods.map((pm) => (
                  <li key={pm.id} className="flex justify-between items-center p-3 border border-gray-100 rounded hover:bg-gray-50">
                    ...
                    <button onClick={() => handleDeletePayment(pm.id)} className="text-red-500 hover:text-red-700 text-sm">
                      Hapus
                    </button>
                  </li>
                ))}
